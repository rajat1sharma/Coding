public class L002 {
    ...
    public static int coinChangePermutation(int[] coins, int tar, int idx, String ans) {
        if (tar == 0 || idx == coins.length) {
            if (tar == 0) {
                System.out.println(ans);
                return 1;
        ...
        if (tar - val >= 0 && val > 0) {
            coins[idx] = -coins[idx];
            count += coinChangePermutation(coins, tar - val, idx + 1, ans + val + " ");
            coins[idx] = -coins[idx];
        }
        // }
        count += coinChangePermutation(coins, tar, idx + 1, ans);
        return count;

    }

    public static int coinChangePermutation2(int[] coins, int tar, String ans) {
        if (tar == 0) {
            System.out.println(ans);
            return 1;
        }
        ...
        for (int i = 0; i < coins.length; i++) {
            ...
            if (tar - val >= 0 && val > 0) {
                coins[i] = -coins[i];
                count += coinChangePermutation2(coins, tar - val, ans + val + " ");
                coins[i] = -coins[i];
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(coinChangePermutation(coins, tar, 0, ""));

        // System.out.println(coinChangePermutation2(coins,tar,""));
